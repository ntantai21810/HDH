				...
				switch (type) {
					...
					case SC_Write: {
						...
						if (id == 1) {
							int bytes;
							if (charCount == -1) {
								bytes = gSynchConsole->Write(intToChar(buffer), 10);
							}
							else bytes = gSynchConsole->Write(temp, charCount);
							machine->WriteRegister(2, bytes);
						}
						//File
						else {
							int bytes;
							if (charCount == -1) {
								bytes = fileSystem->files[id]->Write(intToChar(buffer), 10);
							}
							else bytes = fileSystem->files[id]->Write(temp, charCount);
